...
public class NfcReadWriteActivity extends Activity {
	...
	private static final String LOG_TAG = NfcReadWriteActivity.class
			.getSimpleName();
	@SuppressWarnings("unused")
			...
			a: for (int i = 0; i < techList.length; i++) {
				if (techList[i].contains("MifareClassic")) {
					...
					try {
						...
						autha = Card.mMifareClassic.authenticateSectorWithKeyA(
								j, MifareClassic.KEY_DEFAULT);
						if (canWrite) {
							if (autha) {
								...
								Card.mMifareClassic.writeBlock(writeB,
										writebyte);
								metaInfo += "Sector "
										+ j
										+ ":"
										+ getResources().getString(
												R.string.nfc_block_write)
										+ " "
										+ writeB
										+ " "
										+ getResources().getString(
								...
								metaInfo += "Sector "
										...
										+ getResources()
												.getString(
														R.string.info_validation_failed)
										+ "\n";
							}
							canWrite = false;
						} else {
							if (autha) {
								...
								metaInfo += "Sector "
										...
										+ getResources()
												.getString(
														R.string.info_validation_success)
										+ "\n";
								byte[] data;
								data = Card.mMifareClassic
										.readBlock(blockIndex);
								metaInfo += "Block " + blockIndex + " : "
	...
	public boolean isHexAnd16Byte(String hexString, Context context) {
	...
	public void OnWrite(View view) {
		...
		if (isHexAnd16Byte(data, this) == false) {
			return;
		}
		Toast.makeText(NfcReadWriteActivity.this,
				getResources().getString(R.string.nfc_put_card_again),
				Toast.LENGTH_SHORT).show();
		canWrite = true;
	}
